...
func _ready():
	...
	# FIXED: Automatically use first checkpoint as default spawn, adjusted for proper ground placement
	var ground_spawn_position = Vector2(-101, -80)  # Higher up (lower Y value)
	
	# Try to find the first checkpoint and use its position, but adjust Y coordinate
	...
	if checkpoint_nodes.size() > 0:
		...
		# Adjust the Y position to be much higher (much lower Y value) than the checkpoint
		ground_spawn_position = Vector2(checkpoint_pos.x, checkpoint_pos.y - 40)
